...
class MultKAN(nn.Module):
    ...
    def suggest_symbolic(self, l, i, j, a_range=(-10, 10), b_range=(-10, 10), lib=None, topk=5, verbose=True, r2_loss_fun=lambda x: np.log(1+1e-5-x), c_loss_fun=lambda x: np.exp(x), alpha = 0.1, beta = 1.5):
        '''
    ...
    def auto_symbolic(self, a_range=(-10, 10), b_range=(-10, 10), lib=None, verbose=1, alpha = 0.1, beta = 1.5, r2_threshold=0.0):
        '''
            ...
            for i in range(self.width_in[l]):
                for j in range(self.width_out[l + 1]):
                    if self.symbolic_fun[l].mask[j, i] > 0. and self.act_fun[l].mask[i][j] == 0.:
                        ...
                        name, fun, r2, c = self.suggest_symbolic(l, i, j, a_range=a_range, b_range=b_range, lib=lib, verbose=False, alpha=alpha, beta=beta)
                        if r2 >= r2_threshold:
